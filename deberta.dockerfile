...
# Note: this dockerfile is modified from TensorRT OSS docker: https://github.com/NVIDIA/TensorRT/blob/main/docker/ubuntu-20.04.Dockerfile. Major change is to setup onnxruntime build environment as well.
...
# Setup user account (sudo password: nvidia)
ARG uid=1000
ARG gid=1000
...
RUN pip3 install transformers==4.18 
RUN pip3 install onnx numpy argparse 
RUN pip3 install nvidia-pyindex && pip3 install onnx-graphsurgeon

# Install Cmake (TRT OSS needs 3.14.4, but ORT needs >= 3.18.0. Later upgrade to use 3.21.0, to be consistent with ORT docker: https://github.com/symphonylyh/onnxruntime/blob/master/dockerfiles/Dockerfile.tensorrt)
RUN cd /tmp && \
...
ENV ORT_PATH /workspace/onnxruntime
